...
type UltimateRow = { id: number; name: string; ultimate_skill_url: string | null };
type ApiRes =
  | { ok: true; skills: UltimateRow[] }
  | { ok: false; error?: string };

export default function AdminMasterUltimateSkillsPage() {
  ...
  const [edit, setEdit] = useState<UltimateRow | null>(null);
  ...
  async function createRow() {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/admin/ultimate-skills", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim(), ultimate_skill_url: url.trim() }),
      });
    ...
    try {
      const res = await fetch("/api/admin/ultimate-skills", {
        ...
        body: JSON.stringify({
          ...
          ultimate_skill_url: String(edit.ultimate_skill_url ?? "").trim(),
        }),
      });
      const j = await res.json();
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-3 sm:px-4 md:px-0 py-2">
      <div className="space-y-6">
        <Card>
          <div className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Master Data • Ultimate Skills</div>
          <div className="mt-1 text-sm text-zinc-500">เพิ่ม/แก้ไข Ultimate (name, ultimate_skill_url)</div>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="เช่น Ironclan Ultimate" />
            </div>
            <div>
              <div className="text-xs text-zinc-500 mb-1">Ultimate Skill URL</div>
          ...
          <div className="mt-4 flex justify-end">
            <Button onClick={createRow} disabled={!canSubmitCreate}>
              {creating ? "กำลังเพิ่ม..." : "เพิ่ม Ultimate"}
            </Button>
          </div>
        </Card>
                  ...
                  rows.map((r) => (
                    <tr key={r.id} className="border-t border-zinc-200 dark:border-zinc-800">
                      ...
                      <td className="py-3 pr-3">
                        {r.ultimate_skill_url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img
                            src={r.ultimate_skill_url}
                            alt=""
                      ...
                      <td className="py-3 pr-3 text-zinc-500 break-all">{r.ultimate_skill_url ?? "-"}</td>
        ...
        >
          <div className="space-y-3">
            ...
            <div>
              <div className="text-xs text-zinc-500 mb-1">Ultimate Skill URL</div>
              <Input
                value={edit?.ultimate_skill_url ?? ""}
                onChange={(e) => setEdit((x) => (x ? { ...x, ultimate_skill_url: e.target.value } : x))}
              />
            </div>

            {edit?.ultimate_skill_url ? (
              <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-3">
                <div className="text-xs text-zinc-500 mb-2">Preview</div>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={edit.ultimate_skill_url}
                  alt=""
                  className="h-20 w-20 rounded-2xl border border-zinc-200 dark:border-zinc-800 object-cover bg-white/60 dark:bg-zinc-950/40"
                />
              </div>
            ) : null}
