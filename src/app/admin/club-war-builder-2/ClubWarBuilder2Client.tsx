...
export default function ClubWarBuilder2Client({ canEdit }: { canEdit: boolean }) {
  ...
  const loadClubRoster = useCallback(async () => {
    const res = await fetch("/api/admin/club-roster-2", { cache: "no-store" });
  ...
  useEffect(() => {
    const ch = supabase
      .channel("club-war-builder-2-realtime")
  ...
  const loadPlans = useCallback(async (page: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/club-party-plans-2?page=${page}&pageSize=${pageSize}`, { cache: "no-store" });
  ...
  const savePlan = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/club-party-plans-2", {
        method: "POST",
